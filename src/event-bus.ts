...
export interface IEventBus extends IDisposable {
     ...
     */
    getHandlers<TEvent extends IEvent>(eventName: string): AsyncEventHandler<TEvent>[];
    ...
    /**
     * Registers a new event subscription for the specified event type/name
     * @param eventName
     * A string uniquely identifying the event
     * @param handler
     * A new handler to be invoked when a new event is published
     */
    subscribe<TEvent extends IEvent>(eventName: string, handler: AsyncEventHandler<TEvent>): void;

    /**
     * Removes the event subscription for the specified event type/name
     * @param eventName
     * A string uniquely identifying the event
     * @param handler
     * An existing handler for the current event
     */
    unsubscribe<TEvent extends IEvent>(eventName: string, handler: AsyncEventHandler<TEvent>): void;
